...
import WriteIcon from "./componets/WriteIcon";
...
const App = () => {
  ...
  const userId = "db81ed16-b160-4db2-a046-6309e9b765fe";

  const getUser = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/users?user_uuid=${userId}`
      );
  ...
  const getThreads = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/threads?thread_from=${userId}`
      );
      const data = await response.json();
      setThreads(data);
    } catch (error) {
  ...
  const getReplies = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/threads?reply_to=${interactingThread?.id}`
      );
      const data = await response.json();
